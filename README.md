...
The code sample comes with a working example of a Selenium test procedure that loads a webpage, fills in a search field and looks for the results. To change this with your own Selenium flow, open the `app.py` file and edit lines 18 to 22. Additional documentation on using the Selenium Webdriver Python SDK can be found here: https://selenium-python.readthedocs.io/getting-started.html#simple-usage
...
```bash
seleniumSam$ sam local invoke SeleniumFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
seleniumSam$ sam local start-api
seleniumSam$ curl http://localhost:3000/
```
...
```bash
seleniumSam$ sam logs -n SeleniumFunction --stack-name seleniumSam --tail
```
